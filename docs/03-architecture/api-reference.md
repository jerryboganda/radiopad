...
| GET    | `/api/reports/{id}/ai/jobs/{jobId}` | Poll a job (all kinds): `{ jobId, kind, mode, status: "running"\|"ok"\|"error", elapsedMs, result?, error?, errorKind?, progress? }`. `progress = { tokens, percent }` is present only while `status == "running"` (hot-cache only, omitted otherwise; `percent` is null in v1). Fast request; NOT `ai`-rate-limited. 404 `kind:"job_not_found"` after a server restart (jobs are in-memory; results retained ~15 min). |
| POST   | `/api/reports/{id}/dictation/cleanup/jobs` | PR-B5 durable variant of `/dictation/cleanup` (which stays live): body `{ rawDictation }`, RBAC `Radiologist`/`MedicalDirector`, returns `202 { jobId, status }`. Runs `kind:"ai" mode:"cleanup"`; poll `result` mirrors the sync `{ cleanedSections{…}, provider, model, latencyMs, promptVersion }`. The raw dictation persists in `AiJob.InputJson` (24h null-out) so Retry re-runs it. Suggestion set only — never writes the report. **Rate-limited** (`ai`). |
| POST   | `/api/reports/{id}/crosscheck/review/jobs` | PR-B5 durable variant of `/crosscheck/review` (which stays live): body `{ text, sectionKey?, useUbag? }`, RBAC `Radiologist`/`MedicalDirector`, returns `202 { jobId, status }`. Runs `kind:"crosscheck"`, `mode` = normalized `sectionKey` (or `report`) — **per-section single-flight**. Poll `result` mirrors the sync `{ provider, model, latencyMs, corrections[…] }`. Payload persists in `AiJob.InputJson` (24h null-out). Suggestion set only. **Rate-limited** (`ai`). |
...
| `openai-compatible` | HTTP | `Sandbox` (or `LocalOnly` for `127.0.0.1` / `localhost` / `*.local` hosts) | Generic OpenAI-API-compatible endpoint. **Iter-36:** when `apiKeySecretRef` is set but resolves empty, the adapter throws `ProviderPolicyException("api_key_missing")` instead of letting upstream return 401. Private-network endpoints require `LocalOnly`. The PHI gate was removed (2026-07-20): PHI routes to any enabled provider. |
