...
**Status:** Current  ·  **Owner:** Security + Engineering  ·  **Last Updated:** 2026-06-05
...
| `ai_request_spike` | recent `AiRequest` count ≥ max(20, 10× per-window baseline) | 5 min recent vs 24 h baseline |
...
| Access control via tenant header + reverse proxy | §164.312(a)(1) | Art. 25, 32 |
...
Iter-32 promotes PRD **SEC-003** to ✅ by shipping real cloud KMS adapters.
...
5. `OAuthRefreshRotationService` runs as a hosted background worker on a
   ...
   expiry indicate rotation is due (`before_expiry` ⇒ within 1 h of
   `OAuthRefreshTokenExpiresAt`; `every_24h` ⇒ updated more than 24 h
   ago; `never` ⇒ skipped). The worker delegates the upstream exchange
...
- Storing PHI in browser `localStorage` (the frontend stores only tenant slug
  and user email there).
...
its scope contains `shell:`, `fs:`, or `net:` and either the operator env
