...
**Status:** Draft  ·  **Owner:** Regulatory  ·  **Last Updated:** 2026-05-20  ·  **Iteration:** 48 browser-auth hardening addendum
...
Iteration 47 adds provider/auth hardening evidence without changing the shipped requirement count: hosted endpoint allowlists are enforced before credentials attach, CLI providers require a production binary allowlist, server-side GitHub Copilot CLI execution requires explicit production opt-in, Codex runs through `codex exec --sandbox read-only -`, signed expiring `rp_` bearer tokens replace the older deterministic token shape, and golden rulebook validation now uses the canonical `expectFlagged` contract. Verification evidence: backend `dotnet test --no-build` passed 500/505 with 5 live-infra skips, frontend typecheck/tests/build passed, OpenAPI lint passed as valid with warnings, all 22 rulebooks validated, and all 52 golden cases passed.

Iteration 48 adds browser-auth hardening evidence: magic-link consume now sets an HttpOnly `radiopad_session` cookie for browser sessions, production magic-link requests require SMTP plus `RADIOPAD_PUBLIC_WEB_URL` and never expose raw dev links, client-supplied callback origins are ignored in Production, logout clears the browser session cookie, and SAML/SCIM reverse-proxy routes are documented for external reachability.
...
| CLI-007 | Provider adapters (APIs / local / OAuth) | Iter-31/36 adapters (Azure OpenAI, AWS Bedrock, GCP Vertex, OpenAI direct, OpenAI-compatible generic, GitHub Copilot SDK fail-closed, GitHub Copilot CLI, Gemini CLI, Codex CLI) under `RadioPad.Infrastructure/Providers/`; Iter-47 hardens hosted endpoint allowlists, production CLI binary allowlists, Copilot server CLI opt-in, and Codex `exec --sandbox read-only -` invocation | `ProviderSecretResolver` env-var only; CLI subprocesses scrub environment, refuse PHI/secret prompts, require production binary allowlists, and Codex requires explicit opt-in | ✅ | 5.4 |
...
| PROV-005 | Sandbox model comparison | Sandbox tenants flagged via `Tenant.AllowSandboxRulebooks = true` may register multiple `ProviderConfig` rows with `ProviderComplianceClass.Sandbox`. Iter-34 ships `POST /api/ai/sandbox/compare` (`SandboxCompareController`) — radiologist picks a draft + mode + 1–4 sandbox providers and gets a `runs[]` payload with each output, latency, and token counts side-by-side. UI panel lives on `/providers` (`SandboxComparePanel`) and renders when at least one sandbox provider is configured. PHI policy is still enforced inside `AiGateway.EnforcePhiPolicy`. | iter-34 `Iter34SandboxCompareTests` (4 cases — 409 sandbox flag, 400 mixed-compliance, 200 happy path, PHI + DeIdentifiedOnly refusal); `Iter32AiCompletenessTests`; provider page tests | ✅ | 5.5 |
| PROV-006 | API key vaulting + rotation | `ApiKeySecretRef = "env:<NAME>"` enforced at provider save time and by `ProviderSecretResolver`; inline literals and unsupported schemes resolve to no secret. Rotation = swap env var, no code redeploy. KMS-backed at-rest column encryption applied via `AesGcmColumnEncryptor` + `IKmsProvider` chain (iter-32). | iter-32 `KmsAdapterTests`; `.github/instructions/security.instructions.md` | ✅ | 5.4 |
