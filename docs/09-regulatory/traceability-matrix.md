...
Iteration 47 adds provider/auth hardening evidence without changing the shipped requirement count: hosted endpoint allowlists are enforced before credentials attach, CLI providers require a production binary allowlist, Codex runs through `codex exec --sandbox read-only -`, signed expiring `rp_` bearer tokens replace the older deterministic token shape, and golden rulebook validation now uses the canonical `expectFlagged` contract. Verification evidence: backend `dotnet test --no-build` passed 500/505 with 5 live-infra skips, frontend typecheck/tests/build passed, OpenAPI lint passed as valid with warnings, all 22 rulebooks validated, and all 52 golden cases passed.
...
| CLI-007 | Provider adapters (APIs / local / OAuth) | Iter-31/36 adapters (Azure OpenAI, AWS Bedrock, GCP Vertex, OpenAI direct, OpenAI-compatible generic, Gemini CLI, Codex CLI) under `RadioPad.Infrastructure/Providers/`; Iter-47 hardens hosted endpoint allowlists, production CLI binary allowlists, and Codex `exec --sandbox read-only -` invocation | `ProviderSecretResolver` env-var only; CLI subprocesses scrub environment, refuse PHI/secret prompts, require production binary allowlists, and Codex requires explicit opt-in | ✅ | 5.4 |
