...
## Iter-54 - Wire up the marketing site; restore the marketing/admin domain split

- **Date**: 2026-07-21
- **Trigger**: Operator report — `radiopadstudio.com` wasn't opening the marketing site. Root cause: the `marketing/` Astro site (landing + blog) was fully built but never deployed anywhere — `marketing-static.yml` builds it and uploads a GitHub Actions artifact that nothing downloads; `deploy/vps/nginx.conf` and `docker-compose.yml` have no route for it. The root domain has served the RadioPad admin app (`radiopad-web`) this whole time. This exact marketing/admin split shipped once before under the old domain `radiopad.polytronx.com` (`docs/HANDOFF-desktop-first-surface-split.md`, 2026-07-12) but the 2026-07-13 domain migration to `radiopadstudio.com` was a pure string rename — it never recreated the split on the new domain. Confirmed on the VPS (SSH): NPM proxy host 42 forwards ALL of `radiopadstudio.com`/`www` to `radiopad-web`; no `admin.radiopadstudio.com` NPM host exists (its DNS record resolves but TLS handshakes fail — no vhost/cert); no marketing container exists.
- **Fix**: new `deploy/vps/marketing.Dockerfile` + `nginx-marketing.conf` (pnpm 9.15.9 + Astro build → nginx:alpine) serving the marketing static site at `/`. Critically, the marketing nginx config ALSO proxies `/api /ws /saml /scim` to `radiopad-api` — byte-identical to the admin app's proxy blocks — so already-installed desktop clients (which default to `RADIOPAD_BACKEND=https://radiopadstudio.com`, baked in at build time) keep working on the root domain indefinitely, with zero forced-migration breakage window. New desktop builds default to `https://admin.radiopadstudio.com` instead (`main.rs` `get_backend_url`, `tauri.conf.json` CSP now allow-lists both origins, `frontend/lib/api.ts` `DEFAULT_COMPANION_BASE`). Added a `radiopad-marketing` service to `deploy/vps/docker-compose.yml` and wired its image build into `web-deploy-images.yml` (added `marketing/**` to the trigger paths). Added a "Sign in" link to the marketing nav (`marketing/src/lib/site.ts`) — the site previously had zero path back to the product.
- **VPS-side (operator-executed)**: load the new `radiopad-marketing` image, add it to the live compose file, update `RADIOPAD_PUBLIC_WEB_URL` → `https://admin.radiopadstudio.com` and add `https://admin.radiopadstudio.com` to `RADIOPAD_WEBAUTHN_ORIGINS` (kept the root-domain origins too — `RADIOPAD_WEBAUTHN_RP_ID` stays the parent domain, so existing passkeys keep working per WebAuthn's registrable-domain-suffix rule). In Nginx Proxy Manager: new proxy host for `admin.radiopadstudio.com` → `radiopad-web`, and repoint the existing `radiopadstudio.com`/`www` host to `radiopad-marketing`.
- **Scope**: frontend + desktop + marketing + deploy/vps + docs → desktop release required (DESK-001) after CI green.

## Iter-55 - Report Templates (REPORT-TEMPLATES): per-radiologist output-document designer

- **Date**: 2026-07-26
- **Trigger**: Operator request — radiologists had no way to shape the actual exported PDF/DOCX document (letterhead, branding, section order/typography); `ReportDocumentRenderer` was a single hardcoded layout. Built a full drag-and-drop visual designer, a shared tenant gallery of designs, a mandatory "Powered by RadioPad" branding footer, and a tenant-admin recommendation.
- **Naming**: internal entity/route/permission names use **ReportLayout** throughout — `ReportTemplate`/`/templates`/`templates.*` were already taken by the unrelated clinical section-scaffolding feature. UI label is "Report Templates"; do not conflate the two in code.
- **Backend**: `ReportLayout` + `ReportLayoutUserDefault` entities, `TenantSettings.RecommendedReportLayoutId` (migration `20260726120000_AddReportLayouts`). `ReportLayoutParser` — strict, hand-rolled `LayoutJson` (schema v1) validator/model, 256 KB cap, PNG/JPEG magic-byte-checked logos. `PdfLayoutRenderer` (QuestPDF) + `DocxLayoutRenderer` (OpenXml) render the block model; `ReportDocumentRenderer`'s original hardcoded bodies are frozen as the legacy "Classic" fallback (zero behavior change when no layout resolves) — both new and legacy paths still read section bodies verbatim from `Report`'s six strings, so `FhirDiagnosticReportSerializer.BuildNarrative`/text/FHIR exports are byte-for-byte unchanged. New `ReportLayoutsController` (`/api/report-layouts`) mirrors `FindingsLibraryController`: CRUD is open to any tenant member (shared gallery), mutation restricted to author + governance roles — **no new `RbacPermission`**. `ReportsController` export endpoints gained `?layoutId=` (explicit id/`"classic"` → caller default → tenant recommended → legacy; a stale/unparseable stored layout logs a warning and falls back to legacy rather than failing the export). The "Powered by RadioPad — radiopadstudio.com" branding line is a server-side constant (`ReportLayoutBranding`), never a field the client can set or omit. New `AuditAction.ReportLayoutChanged = 133`.
- **Frontend**: `frontend/lib/reportLayouts/` (schema/validator mirroring the C# parser, 4 built-in presets, undo/redo reducer capped at 50 steps, device-local zoom/last-export prefs). `/report-templates` gallery (card grid, Recommended/My-default badges, preset picker) and `/report-templates/designer` (three-zone: block palette with the same drag-and-drop as the rulebook editor, `overflow:auto`-wrapped canvas rendering the shared `LayoutPaper` component, inspector with per-block forms incl. client-side logo downscale-to-PNG). `ExportPanel` gained a Layout selector threading `layoutId` into `api.reports.exportPdf/exportDocx`.
- **Tests**: `ReportLayoutPackTests.cs` (CRUD, ownership 403, cross-tenant 404, six validation-rejection cases, default/recommended set-clear-cascade, preview no-side-effects + branding-text assertion, export layoutId resolution incl. unacknowledged-report 409, legacy-content behavior-compat). Frontend `layoutReducer.test.ts` + `reportLayoutPresets.test.ts` (every preset passes validation). `sidebar.test.tsx` LOCKED_GROUPS updated in the same change.
- **Scope**: backend + frontend + docs → desktop release required (DESK-001) after CI green.
- **Follow-up fixes (same day, after operator feedback on the shipped designer)**:
  - `LayoutPaper`'s wrapper hard-clipped to exactly one page's height (`overflow: hidden`) — any layout whose content ran longer than one physical page (the normal case) had everything past that height, including the branding footer, silently cut off in the designer canvas. Added a `clip` prop (default true, right for the small fixed-aspect gallery/preset thumbnails); the canvas now passes `clip={false}` so it shows the full document, matching what a real export would actually produce (flow onto page 2, not drop content).
  - The footer was up to three stacked lines (status/custom, branding, page number) in all three renderers. Collapsed to one line with bullet separators everywhere (canvas preview, PDF, DOCX).
  - Added a second mandatory, non-removable footer element — **"NOT VALID IN COURT OF LAW"** — with the same server-enforced contract as the branding line (a backend constant, never a client-editable `LayoutJson` field). Both the disclaimer and the branding line now render bold in all three renderers.
  - **Naming collision found and fixed**: the `RPT-030` tag used throughout comments/docs for this feature collided with an existing, unrelated PRD v2 requirement — `RPT-030 Multi-study reporting view (combined exam reads)` — in `docs/09-regulatory/prd-v2-traceability-matrix.md`. Renamed the internal tag to `REPORT-TEMPLATES` across all 34 files that used it (backend, frontend, docs, tests); the 3 legitimate PRD/matrix source documents were left untouched since `RPT-030` there correctly refers to the other requirement.
  - Deployed to production (`admin.radiopadstudio.com` / SQLite at `/data/radiopad.db` on the VPS) and verified end-to-end: `ReportLayouts`/`ReportLayoutUserDefaults` tables present with correct schema, `GET /api/report-layouts` live (401 not 404), `TenantSettings.RecommendedReportLayoutId` column present.

## Iter-56 - Patient age: Years/Months unit on the intake composer

- **Date**: 2026-07-27
- **Trigger**: Operator report — the Age field in the new-report wizard (`/reports/new` step 1) only accepted a plain number, which reads as years; an infant's age (e.g. "6 months old") had no way to be entered correctly, so it would silently be misread as "6 years" by the AI provider.
- **Backend**: `StudyContext` gained `AgeUnit` (new `RadioPad.Domain.Enums.StudyAgeUnit`: `Years` default / `Months`) alongside the existing `Age` int, migration `20260727160156_AddStudyAgeUnit`. `CreateReportDto`/`PatchReportDto` (`ReportsController`) and `GenerateReportDto`/`GenerateReportJobDto` (`LocalGenerationController`, on-device path) each gained a matching `AgeUnit` string, parsed case-insensitively ("Months" → Months, anything else → Years — same free-form convention as `Gender`/`Contrast`). The prompt header patient line ("Patient: age …") in both `ReportingService.BuildPromptForMode` call sites and the local-generation `BuildUserPrompt` now renders `"{n} years"`/`"{n} months"` instead of a bare number, via a shared `FormatAge` helper in each file — so the AI provider always receives the radiologist's actual selected unit, not an ambiguous integer. `ReportLayoutContent.ResolveStudyFieldValue`'s `Age` field (PDF/DOCX study-info box) appends `" mo"` for months so an exported document doesn't misread an infant's age either.
- **Frontend**: `NewReportWizard.tsx` step 1 splits the single Age input into the existing number field plus a Years/Months `<select>` (defaults to Years; switching units clears the value rather than silently reinterpreting it — e.g. a typed "54" would otherwise become "54 months"). Max bound tightens to 23 for Months (150 for Years). `ageUnit` threads through `api.reports.create` and the local-generate job `dto` alongside `age`. `frontend/lib/api.ts` types (`Report['study'].ageUnit`, the `patch` body, `LocalGenerateDto`) updated to match.
