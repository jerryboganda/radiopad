...
- PHI gate removed (operator decision 2026-07-20): `AiGateway` no longer blocks requests by `ProviderComplianceClass` or PHI content — only the per-provider `Enabled` toggle applies. Compliance classes are informational metadata. Do not reintroduce the gate without an explicit operator request.
