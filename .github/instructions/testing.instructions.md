...
- Approved rulebooks (`status: approved`) require at least one passing golden case under `rulebooks/_tests/<rulebook_id>/`. Golden cases are JSON fixtures with `report` + `expectedFindings`.
