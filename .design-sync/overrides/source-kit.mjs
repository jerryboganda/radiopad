// forked from design-sync lib/source-kit.mjs — synth entry restricted to componentSrcMap
// pins + browser process.env shim (RadioPad is a Next.js app, not a library: the unpinned
// synth entry pulled all 57 component files incl. router/data-coupled ones, and Next-style
// modules read process.env at module init, which breaks the bundle in a plain browser).
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // Fork: when componentSrcMap pins exist, the synth entry carries ONLY the
    // pinned files — the unpinned scan would bundle every app component
    // (router/auth/data-coupled ones included) and their import-time hazards.
    const pinned = new Set(
      Object.values(srcMap)
        .filter((v) => typeof v === 'string')
        .map((p) => slash(resolve(PKG_DIR, p)).toLowerCase()),
    );
    const all = srcFiles.filter((p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p));
    const comps = pinned.size ? all.filter((p) => pinned.has(slash(p).toLowerCase())) : all;
    // Fork: Next-style modules (lib/api) read process.env.* at module init;
    // esbuild only defines NODE_ENV, so a plain browser throws "process is not
    // defined" and the whole global dies. Ship a first-import env shim —
    // mirrors what Next's own build does with compiled-in env.
    const shim = join(OUT, '.env-shim.mjs');
    writeFileSync(shim, 'if (!globalThis.process) globalThis.process = { env: {} };\nif (!globalThis.process.env) globalThis.process.env = {};\n');
    entry = join(OUT, '.pkg-entry.mjs');
    // Fork: RadioPad components are mostly `export default function X` — a bare
    // `export * from` chain drops default exports (ES star-export semantics),
    // leaving window.<GLOBAL>.<Name> undefined. Emit an explicit
    // `export { default as Name }` for every pinned name whose file has a
    // default export and no same-named named export.
    const lines = [`import './.env-shim.mjs';`];
    for (const f of comps) {
      lines.push(`export * from ${JSON.stringify(f)};`);
      const text = readText(f);
      if (!/export\s+default\b/.test(text)) continue;
      const namesForFile = Object.entries(srcMap)
        .filter(([, v]) => typeof v === 'string' && slash(resolve(PKG_DIR, v)).toLowerCase() === slash(f).toLowerCase())
        .map(([k]) => k);
      for (const name of namesForFile) {
        if (new RegExp(`export\\s+(?:const|let|var|function|class)\\s+${name}\\b`).test(text)) continue;
        lines.push(`export { default as ${name} } from ${JSON.stringify(f)};`);
      }
    }
    writeFileSync(entry, lines.join('\n') + '\n');
    synthEntry = true;
    console.error(
      `[NO_DIST] no built entry — synthesizing from ${comps.length} src files (${pinned.size ? 'componentSrcMap-pinned' : 'unpinned scan'})`,
    );
  }
  ...
  // Fork: RadioPad's styling is five plain CSS files loaded in app/layout.tsx
  // order (globals → tokens → motion → radiopad → shell). shell.css ships as
  // cfg.cssEntry (= _ds_bundle.css, imported LAST by styles.css); the first
  // four ride this adapter tokensCss lane (imported first, in this order), so
  // the emitted closure preserves the app's cascade order exactly.
  const tokensCss = ['app/globals.css', 'app/tokens.css', 'app/motion.css', 'app/radiopad.css']
    .map((p) => resolve(PKG_DIR, p))
    .filter((p) => existsSync(p));
  return { shape: 'package', entry, components, synthEntry, exported, tokensCss };
}
